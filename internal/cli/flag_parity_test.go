...
var knownFlagGaps = map[string][]string{
	...
	"up":     {"always-recreate-deps", "attach", "attach-dependencies", "build", "menu", "no-attach", "no-color", "no-log-prefix", "quiet-build", "quiet-pull", "renew-anon-volumes", "timestamps", "watch", "yes"},
	"watch":  {"prune"},
}

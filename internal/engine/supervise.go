...
// Supervise watches the restart-enabled containers of the named services (or
// all services) and restarts them according to their policy when they exit. It
// returns when every watched container has reached a terminal state or the
// context is cancelled. It is intended for a foreground `up`.
func (e *Engine) Supervise(ctx context.Context, p *types.Project, names []string) error {
	if len(names) == 0 {
